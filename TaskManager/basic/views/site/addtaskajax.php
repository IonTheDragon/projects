	...
	$( "#add_task_form" ).submit(function( event ) {
		  ...
		  $.ajax({
			...
			success:function(data){
                ...
                $( "#task_table" ).append("<tr id=tr"+json_data.id+"><td>"+json_data.title+"</td><td>"+json_data.user+"</td><td>"+json_data.status+"</td><td>"+json_data.description+"</td><td><button type=button onclick=edit_task_form("+json_data.id+");>Edit</button></td><td><button type=button onclick=delete_task("+json_data.id+");>Delete</button></td></tr>" );
			},
			error:function(data){
                alert(data[0]); 
                $('#newform').html(''); 
